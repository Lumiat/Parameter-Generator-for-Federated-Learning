...
# 使用 FederatedDataset 加载 CIFAR10 数据集，并通过 IidPartitioner 进行 独立同分布（IID）分区，模拟联邦学习中的客户端数据分布。

# 数据预处理包括归一化（Normalize）和转换为张量（ToTensor）。
...
def load_data(partition_id: int, num_partitions: int, alpha: float, batch_size: int):
    """Load partition CIFAR100 data. Can be iid or non-iid."""
    # Only initialize `FederatedDataset` once
    global fds
    if fds is None:
        partitioner = DirichletPartitioner(num_partitions=num_partitions, alpha=alpha, partition_by="fine_label")
