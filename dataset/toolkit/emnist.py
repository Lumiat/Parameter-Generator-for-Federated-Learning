...
# 使用 FederatedDataset 加载 EMNIST 数据集，并通过 DirichletPartitioner 进行non-iid分区，模拟联邦学习中的客户端数据分布。

# 数据预处理，转换为张量（ToTensor）
...
def load_data(partition_id: int, num_partitions: int, alpha: float, batch_size: int):
    """Load partition EMNIST data. Can be iid or non-iid."""
    # Only initialize `FederatedDataset` once
    global fds
    if fds is None:
        partitioner = DirichletPartitioner(num_partitions=num_partitions, alpha=alpha, partition_by="label")
        fds = FederatedDataset(
            dataset="randall-lab/emnist",
            subset="byclass",
            partitioners={"train": partitioner},
            seed=None,
            trust_remote_code=True
        )
    assert isinstance(batch["image"][0], (PIL.Image.Image, np.ndarray)), "Invalid image type"
