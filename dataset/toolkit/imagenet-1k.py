...
from torchvision.transforms import Resize, RandAugment, Compose, RandomCrop, RandomHorizontalFlip, Normalize, ToTensor
# from command import parser

fds = None  # Cache FederatedDataset

# 使用 FederatedDataset 加载 ImageNet-1K 数据集，并通过 IidPartitioner 进行 独立同分布（IID）分区，模拟联邦学习中的客户端数据分布。

# 数据预处理，做数据增强，转换为张量（ToTensor），
...
def load_data(partition_id: int, num_partitions: int, alpha: float, batch_size: int):
    """Load partition Fashion-MNIST data. Can be iid or non-iid."""
